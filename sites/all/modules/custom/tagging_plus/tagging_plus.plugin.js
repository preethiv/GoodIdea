  ...
  $.fn.tagging_plus = function() {
    return this.each( function(){
      ...
      */
      ...
      /*
      * Event for keypress on the input field of the tagging_plus-widget.
      */
      function bind_enter() {
      ...
      * This is extracted from the tagging_plus-widget-XX class
      * if the input element, which actuall has the tagging_plus-widget class.
      */
      function get_context(classes) {
        context = null;
        $(classes.split(' ')).each(function(){
          match = this.match(/tagging_plus[-]widget[-]input[-](\d+)/i);
          if (match != null) {
            context =  match[1];
          }
        });
