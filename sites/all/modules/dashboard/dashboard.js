...
Drupal.behaviors.dashboardNavigation = function(context) {
  ...
  if ($navigation.length > 0) {
    ...
    // Drag & drop tabs
    $navigation.sortable({
      items: '>ul>li:not(.dashboard-link-add, .dashboard-profile, .dashboard-widgets, .0)',
...
Drupal.behaviors.dashboardWidget = function(context) {
  ...
  $("#dashboard>div.column>div.widget:not(.dashboard-processed)", context).addClass('dashboard-processed').each(function() {
    ...
    $('a.minimize-widget', $this).data('widget', $this).click(function () {
      //console.log($this.children('div.content'));

      var $widget = $(this).data('widget');

      if($this.children('div.content').is(':hidden')) {
        $widget.children('div.content').slideDown('fast');
      } else {  
        $widget.children('div.content').slideUp('fast');
      }

      jQuery.post(Drupal.settings.basePath + 'dashboard/' + Drupal.settings.dashboardPage + '/minimize-widget', {
        token: Drupal.settings.dashboardToken,
    ...
    $('a.change-widget-page', $this).data('widget', $this).click(function() {
      $(this).data('widget').find('ul.page-list').toggle();
      return false;
    });
    $('ul.page-list>li>a', $this).data('widget', $this).click(function() {
      $this.slideUp('fast');
      jQuery.post(Drupal.settings.basePath + 'dashboard/' + Drupal.settings.dashboardPage + '/change-widget-page', {
        token: Drupal.settings.dashboardToken,
        widget_id: $this.attr('id').replace(/^widget-/, ''),
        page_id: $(this).attr('class').replace(/^dashboard-page-/,''),
      });
      return false;
    });
  });
